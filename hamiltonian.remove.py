...
#用于构造哈密顿量各个部分的函数。这
矩阵仍然需要与适当的耦合相乘  cu-o杂化参数tpp tpd ...
'''
import parameters as pam #参数
import lattice as lat  #晶格
import variational_space as vs  #变分空间
import utility as util    #效用
import numpy as np       #数学基础库
import scipy.sparse as sps    #也是一个数学基础库 包括稀疏矩阵 （0元素比非0元素要多 且非0元素无规律）

directions_to_vecs = {'UR': (1,1,0),\    #定义跃迁的方向 往哪个方向跃迁
                      'UL': (-1,1,0),\
                      'DL': (-1,-1,0),\
                      'DR': (1,-1,0),\
                      'L': (-1,0,0),\
                      ...
                      'mzU': (0,1,-1),\
                      'mzD': (0,-1,-1)}
tpp_nn_hop_dir = ['UR','UL','DL','DR']  #四个O在上下左右 O-O跃迁只能斜着

#四种模型可能性 3 7 9 11
#定义tpd的方向 CU-O跃迁的方向
def set_tpd_tpp(Norb,tpd,tpp,pds,pdp,pps,ppp):  #p表示pai s表示sigma
    # dxz and dyz has no tpd hopping  对应表中没有ex ey
    if pam.Norb==7:
        ...
        tpd_nn_hop_dir = {'d3z2r2': ['L','R','U','D'],\  #往哪个方向跃迁
                          'dx2y2' : ['L','R','U','D'],\
                          'dxy'   : ['L','R','U','D'],\
                          'px1'   : ['L','R'],\  #怎么往右边跃迁呢 CU在O的左边
                          'py1'   : ['L','R'],\    #？CU-O跃迁是互相的 可以是CU往O 也可以是O往CU
                          'px2'   : ['U','D'],\
                          'py2'   : ['U','D']}
    elif pam.Norb==11:
        tpd_nn_hop_dir = {'d3z2r2': ['L','R','U','D'],\
    ...
    ## 孔语言：遵循 H.Eskes 的 PRB 1990 论文中图 1 中的符号约定
    # 或 PRB 2016：表征三轨道哈伯德模型...
    # 或者查看 George 在 2021 年 8 月 19 日的电子邮件：
    # dx2-y2 希望 O px 在正 x 方向上应该对电子和负 x 的 O 都是正的
    # 电子的方向应该是负的，即跳跃积分应该是重叠积分的减号
    # 在两个相邻原子之间。

#四种模型tpd的一些可能性 对应表格1
    if pam.Norb==7:
        # d3z2r2 has +,-,+ sign structure so that it is - in x-y plane  所以d3z2r2跃迁的值跟表格上少一个负号
        ...
        c = np.sqrt(3)/2.0
        tpd_nn_hop_fac = {('d3z2r2','L','px1'): -pds/2.0,\   #。CU不是在O的左边吗 向左跃迁是如何跃迁的
                          ('d3z2r2','R','px1'):  pds/2.0,\  #向右跃迁 关于d3z2-r2的值跟表格上少一个负号 解释看一开始的标注
                          ('d3z2r2','U','py2'):  pds/2.0,\
                          ('d3z2r2','D','py2'): -pds/2.0,\    #向下如何跃迁
                          ('dx2y2','L','px1'):   pds*c,\
                          ('dx2y2','R','px1'):  -pds*c,\  #向右跃迁 跟表格一致
                          ('dx2y2','U','py2'):   pds*c,\
                          ('dx2y2','D','py2'):  -pds*c,\
                          ('dxy','L','py1'):  -pdp,\
                          ('dxy','R','py1'):   pdp,\
                          ('dxy','U','px2'):   pdp,\
                          ('dxy','D','px2'):  -pdp,\
                          # below just inverse dir of the above one by one 反过来为啥是一样的
        ...
        c = np.sqrt(3)/2.0
        tpd_nn_hop_fac = {('d3z2r2','L','px1'): -pds/2.0,\   #同样 d3z2r2跟表格上少一个负号
...
#四种模型下 tpp的可能性
    if pam.Norb==7:
        tpp_nn_hop_fac = {('UR','px','py'): -tpp,\    #为啥会出现UR
                          ('UL','px','py'):  tpp,\
                          ('DL','px','py'): -tpp,\
                          ('DR','px','py'):  tpp}
    elif pam.Norb==9:
        tpp_nn_hop_fac = {('UR','px1','px2'):  0.5*(ppp-pps), #根据图看跃迁方向 为什么总会有一个相反的呢
                          ...
                          ('DL','px1','py2'): -0.5*(ppp+pps),\ #为什么相反的方向又出现负号 上面没有出现呢
    ...
    return tpd_nn_hop_dir, tpd_orbs, tpd_nn_hop_fac, tpp_nn_hop_fa2
        #return 把执行的结果 返回到调用的地方 tpd orbs是参与CU-O跃迁的轨道
...
def get_interaction_mat(A, sym):
    ...
    #获取 d-d 库仑和交换相互作用矩阵
    基于lat.spin_int的total_spin：up：1 和 dn：0

旋转 90 度，x 到 y，实际上 y 到 -x，这样基本上可以互换
    两个孔的空间波函数可以引入-符号（例如（DXZ，DYZ））。
    但是我们必须看看这种旋转对两个孔的slater行列式状态有什么影响。
    请记住，三元组状态是 （|up，down> +|down，up>）/sqrt2 所以在互换孔时
    自旋部分保持正值，因此空间部分必须为负值。
    对于交换电子的单重态，自旋部分改变符号，因此空间部分可以保持不变。

在同一空间波函数中的两个空穴上不能出现三元组，而只有单胞胎可以
    但是，如果两个空穴具有正交的空间波函数，则单重态和三胞胎都可能出现
    并且它们的能量会因交换条款而有所不同

ee 表示 xz，xz 或 xz，yz 取决于 o
    '''

#两个空穴都在CU上时 有一个相互作用 有不同的对称性 不同对称性下相互作用的矩阵
    B = pam.B
    C = pam.C
    
    # not useful if treat 1A1 and 1B1 as correct ee states as (exex +- eyey)/sqrt(2)
    # 如果将 1A1 和 1B1 视为正确的 ee 状态，则没有用 （exex +- eyey）/sqrt（2）
    #A B C是库伦矩阵和交换矩阵的拉卡参数 B=0.15ev C=0.58ev  不可约表示
    #以下是两个空穴在cu上相互作用的不可约表示 对应表2


    if sym=='1AB1':
        fac = np.sqrt(6)   #某个参数 根号6
        Stot = 0  #stot是啥意思v
        Sz_set = [0]   #这是啥意思
        state_order = {('d3z2r2','d3z2r2'): 0,\     #状态命令
                       ('dx2y2','dx2y2')  : 1,\
                       ('dxy','dxy')      : 2,\
                       ('dxz','dxz')      : 3,\
                       ('dyz','dyz')      : 4,\
...
def set_matrix_element(row,col,data,new_state,col_index,VS,element):   #辅助
    '''
...
参数
    ----------
    row：包含行索引的 Python 列表
    col：包含列索引的 Python 列表
    data：包含非零矩阵元素的 Python 列表
    col_index：要追加到 col 的列索引
    new_state：与行索引对应的新状态
        附加。
    VS：模块 variationalSpace 中的 VariationalSpace 类
    元素：（复杂）要附加到数据的矩阵元素。

返回
    -------
    None，但将值追加到 row、col、data 中。
    '''

    row_index = VS.get_index(new_state)  #行索引等于变分空间中获得与行索引对应的新状态  是一个新状态
    if row_index != None:  #如果增加一个新状态 数据增加一个元素 行列都增加新索引
        data.append(element)
        row.append(row_index)
        col.append(col_index)

def create_tpd_nn_matrix(VS, tpd_nn_hop_dir, tpd_orbs, tpd_nn_hop_fac): #p轨道与d轨道跃迁
    '''
...
返回
    -------
    矩阵：（sps coo 格式）t_pd哈密顿量的跳跃部分，没有
        前因t_pd。   coo格式 csr csc格式用来储存稀疏矩阵

sps 文档中的注释
    -------------------------------
    默认情况下，转换为 CSR 或 CSC 格式时，重复 （i，j）
    参赛作品将汇总
    '''    
...
#CU-O跃迁矩阵 对应Kpd 两个空穴分开看
    ...
    for i in range(0,dim):
        start_state = VS.get_state(VS.lookup_tbl[i]) #态是在变分空间
        
        # double check which cost some time, might not necessary
        assert VS.get_uid(start_state) == VS.lookup_tbl[i]
        #声明函数 声明某些条件 一旦条件不符合 程序强制退出
        s1 = start_state['hole1_spin']
        s2 = start_state['hole2_spin']
        orb1 = start_state['hole1_orb']
        orb2 = start_state['hole2_orb']
        stax1, y1, z1 = start_state['hole1_coord']
        x2, y2, z2 = rt_state['hole2_coord']

        # hole 1 hops: some d-orbitals might have no tpd
        if orb1 in tpd_orbs:  #空穴1跃迁 orb1是跃迁前的
            for dir_ in tpd_nn_hop_dir[orb1]: #空穴1轨道cu-o跃迁的方向
                vx, vy, vz = directions_to_vecs[dir_]  #dir_矢量的方向
                orbs1 = lat.get_unit_cell_rep(x1+vx, y1+vy, z1+vz)  #orbs1是跃迁后的轨道
                if orbs1 == ['NotOnSublattice']:   #lattice文件里
                    continue
                ...
                # consider t_pd for all cases; when up hole hops, dn hole should not change orb
                 #当向上的空穴跳跃时，下面的空穴没有改变轨道   这个上下是自旋方向还是坐标位置在上下
                for o1 in orbs1: #若o1在跳跃后的轨道上
    ...
    ## 检查（up，up）、（dn，dn）和（up，dn）组内是否发生跳跃
    out = sps.coo_matrix((data,(row,col)),shape=(dim,dim))
    
    return out


#O-O跃迁矩阵 对应Kpp 两个空穴分开看
def create_tpp_nn_matrix(VS,tpp_nn_hop_fac):  #pp轨道跃迁
    '''
    ...
    for i in range(0,dim):
        ...
        # hole1 hops: only p-orbitals has t_pp  跃迁单个粒子 与相互作用不一样
        if orb1 in pam.O_orbs: 
            for dir_ in tpp_nn_hop_dir:
                vx, vy, vz = directions_to_vecs[dir_]
                orbs1 = lat.get_unit_cell_rep(x1+vx, y1+vy, z1+vz)  #跳跃后的空穴1
...
#创建位点能的矩阵 对应Es
def create_edep_diag_matrix(VS,A,ep):  #创建ed ep对角矩阵 对应哈密顿里的ES
    '''
    ...
    row = []
    col = []
    idxs = np.zeros(dim)  #一维数组全是0

    for i in range(0,dim):
        diag_el = 0.  #el是什么意思 是总的能量？ i是什么意思
        state = VS.get_state(VS.lookup_tbl[i]) #lookup-tbl表示包括所有态的列表
        ...
        if orb1 in pam.Ni_orbs and orb2 in pam.O_orbs:
            diag_el += pam.ed[orb1] + ep   #+=符号 表示符号左右两边相加 结果赋值于左边 数字就是直接相加 字符串就是拼接
            idxs[i] = 1   #idxs是什么意思
    ...
    for i in range(0,dim):
        ...
        if (x1,y1) == (x2,y2):
            if orb1 in pam.Ni_orbs and orb2 in pam.Ni_orbs:
                ...
                p_list.append(i)  #获取两个空穴都在CU或者O的列表 列表增加
...
#创建相互作用矩阵 d_double p_double 跟上面状态列表有关吗？ 对应Vdd Vpp
def create_interaction_matrix_ALL_syms(VS,d_double,p_double,S_val, Sz_val, AorB_sym, A, Upp):
    ...
    创建 d 多重态的库仑交换相互作用矩阵，包括所有对称性

遍历所有d_double状态，找到对应的符号通道;
    另一个遍历所有 d_double 状态的循环，如果它具有相同的符号通道和 S、Sz
    进入 Matrix 元素

由于三个空穴和一个钕电子，存在一些并发症或限制：
    从H_matrix_reducing_VS文件中，设置状态 i 和 j 之间的交互：
    1. i 和 j 属于同一类型、相同顺序的轨道来标记状态（idxi==idxj 下）
    2. I 和 J 的自旋相同;或者 L 和 s 也应该具有相同的自旋
    3. L和Nd电子的位置也应该相同
    '''    
    ...
    for sym in channels:
        ...
        #print "orbitals in sym ", sym, "= ", sym_orbs 系统轨道
        ...
        for i in d_double:
            ...
            o12 = sorted([io1,io2])  #sort排序函数
            o12 = tuple(o12) #元组 不能修改的列表
